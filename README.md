...
> 当前处于 **Phase 6（持久化 + 打磨）** 完成阶段：画布批注、侧边栏导航、AI 书签、自由元素、.pdfrx 批注包持久化、套索选择与 PDF 导出均已可用（见[路线图](#路线图)）。
...
- `.pdfrx` 批注包保存/打开：把原始 PDF、全部墨迹（页面 + 自由画布）、文本框、图片与书签打包为单文件，重新打开完整恢复画布状态（缩放、平移、元素、书签）
...
覆盖 PDF 加载、按 DPI 渲染、文本提取、书签目录、缩略图渲染、LLM 书签 JSON 解析、坐标变换（当前 26/26 通过）。
