...
> **v1.0.0** 已发布：画布批注、侧边栏导航、AI 书签、自由元素、.pdfrx 批注包持久化、套索选择与 PDF 导出均已可用（见[路线图](#路线图)）。
...
- LLM 自动生成书签：每批 10 页轻量探测目录（只发当前批，降低 token 消耗），找到后读取完整目录并核对章节偏移推算书签，不再全本读取；全书找不到目录时停止并提示
- 扫描版 PDF 支持：检测到无文字层时，逐批渲染页面图片交给视觉模型找目录，读完目录后定位章节页码确认偏移
- 超长目录自动续传：AI 输出被截断时保留已解析条目，续传只带已保存条目（不重发图片）快速合并
- LLM 请求强制 JSON 输出并带解析容错，避免模型输出解释文字导致误判无目录
- LLM 设置：API 地址 / Key / 文本模型 / 视觉模型可配置，保存在本机 `%AppData%\PDFReaderX\llm-settings.json`
- 图片插入：工具栏「图片」按钮、Ctrl+V 粘贴剪贴板图片、直接把图片文件拖入画布（可拖动、右下角手柄缩放、Delete 删除）
- 自由文本框：「文本」工具点击画布输入，回车确认 / Esc 取消，双击已有文本可重新编辑（可拖动与缩放）
- 元素选择：选择工具点击元素显示虚线框与缩放手柄，点击空白取消选择
- 撤销/重做：墨迹笔划、橡皮擦、元素增删/移动/缩放全部可撤销（Ctrl+Z / Ctrl+Y / 工具栏按钮）
- 套索选择：圈选墨迹笔划，Delete 删除选中笔划（可撤销）
- `.pdfrx` 批注包保存/打开：把全部墨迹（页面 + 自由画布）、文本框、图片与书签打包为单文件，重新打开完整恢复画布状态（缩放、平移、元素、书签）；另存为内嵌原始 PDF，退出自动保存为引用式（轻量，不重复内嵌）
- 退出保存流程：关闭时有未保存修改会提示保存（默认保存），带进度窗口；保存失败可取消退出
- 导出 PDF：把页面墨迹与元素合并渲染（150 DPI）导出为新的 PDF 文件

### 规划中

- 墨迹与元素导出为 PDF 注释（当前为合并渲染导出）
...
覆盖 PDF 加载、按 DPI 渲染、文本提取、书签目录、缩略图渲染、LLM 书签 JSON 解析、坐标变换（当前 27/27 通过）。
...
# 安装包：dist/installer/PDFReaderX-Setup-1.0.0.exe（需要先安装 Inno Setup 6）
